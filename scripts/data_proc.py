...
def stip_segm2box_wrapper():
  inst_root = '/cvgl2/u/ashenoi/STIP/instances_20fps/stip_instances/'
  for vid in sorted(glob(os.path.join(inst_root, '*_*'))):
...
def prepare_data():
  # object GT directory
  obj_root = '/sailhome/bingbin/crossing/datasets/JAAD_instance_segm'
  fobj_dir_format = os.path.join(obj_root, 'video_{:04d}')
  # pedestrian GT files
  ped_root = '/sailhome/bingbin/crossing/datasets/JAAD_dataset/'
  ...
  annot_root = '/sailhome/bingbin/crossing/datasets/'
  # Train
